$(function () {
    ...
    $('#pro_count').keypress(function (ev) {
        if (ev.charCode == 13 && $('#pro_id').val() && $('#pro_count').val()) {
            ...
            storage_in_add(url = '/storage/storage_in_add/', data = $data)
        }
            ...
            if (ev.charCode == 13) {
                if ($('#edit_count_' + $pro_id).val() && ($('#edit_count').val() !== $old_count)) {
                    var $data = {
                        ...
                        'pro_old_count': $old_count
                    };
                    modify_storage_count('/storage/storage_in_detail_modify/', $data);
    ...
    $('.storage_in_modify').on('click', '.del_pro', function () {
        var $pro_id = $(this).parent().attr('pro_id');
        var $pro_count = $(this).parent().attr('pro_count');
        var $data = {
            'pro_id': $pro_id,
            'pro_count': $pro_count
        };
        del_storage_pro('/storage/storage_in_detail_del/', $data);
    });


    $('#submit_storage_in').click(function () {
        window.location.href = '/storage/storage_in_confirm/'
    });
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.code == '888' || result.code == '000') {
                    ...
                    var $append_product = '<tr id="line_pro_id_' + data.pro_id + '" class="storage_in_detail_line storage_in_pro_id_' + data.pro_id + '">' +
                    ...
                    if ($('.storage_in_pro_id_' + data.pro_id).length >= 2) {
                        $('.storage_in_pro_id_' + data.pro_id).css('color', 'red')
                    }
                    $('#pro_id').val('');
                    $('#pro_count').val('');
                    $('#pro_id')[0].focus();
